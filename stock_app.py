...
raw_ticker = st.sidebar.text_input("輸入股票代碼", value="0700.HK").upper().strip()
...
* **港股**：輸入4位數字加 `.HK`，如 `0700.HK` (騰訊), `9988.HK` (阿里), `3690.HK` (美團)
""")
...
if train_button:
    ...
    with st.spinner(f"⏳ 正在拉取全球聯動數據，並為 {raw_ticker} 建立漲跌幅矩陣..."):
        try:
            ...
            if df.empty:
                st.error(f"❌ 找不到代碼「{raw_ticker}」，港股請記得加 `.HK`（例如 0700.HK）")
            else:
                ...
                # 🔥 強力修復：將所有無限大 (inf) 和負無限大 (-inf) 替換為 NaN，然後用 0 填補
                ...
                col1.metric(label=f"今日收盤價 ({raw_ticker})", value=f"{currency_symbol}{current_price:.2f}")
    ...
    st.info("💡 請輸入美股代碼（如 TSLA）或港股代碼（如 0700.HK），並點擊按鈕進行分析。")
